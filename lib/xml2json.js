...
module.exports = function(xml, _options) {
    ...
    if (!parser.parse(xml)) {
        console.log('-->' + xml + '<--');
        if (options.fatalErrors) throw new Error('There are errors in your xml file: ' + parser.getError());
        else console.log('There are errors in your xml file: ' + parser.getError());
    }
